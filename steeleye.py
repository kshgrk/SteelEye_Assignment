...
class Steeleye():
    '''
    Contains methods to download XML data from given URL and then transform the same into custom CSV
    '''
    ...
    def load_initial_xml(self):
        ...
        try:
            ...
            self.xml_content = str(resp.content).replace('\\n', '')\
                                .replace("'","")\
                                .replace("b<","<")
            self.logger.info("Parent XML Loaded Successfully")
        except (requests.exceptions.RequestException, KeyError) as exception:
            self.logger.warning("Parent XML Failed To Load")
    ...
    def extract_download_link(self):
        ...
        try:
            ...
            for item in response.findall('result/doc/str'):
                if item.text.__contains__('http') and item.text.__contains__('DLTINS'):
                    self.xml_zip_url = item.text
